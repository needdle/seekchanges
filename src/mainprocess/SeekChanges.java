...
public class SeekChanges {
	public static long run(String outputFilePath, String modelType, double propAlpha, double propRho, double lambda) throws Exception{
...
//	public static String getPackage(ArrayList<String> file)
	public static void main(String []args) throws Exception{
		if(args.length!=5){
			System.out.println("the number of input parameter is wrong!");
			return;
		}
		String datasetsDirPath = args[0];
		String experimentDirPath = args[1];
		
		double propAlpha = Double.parseDouble(args[2]);
		double propRho = Double.parseDouble(args[3]);
		double lambda = Double.parseDouble(args[4]);
		
		String timestatsFilePath = Paths.get(experimentDirPath,"timeStats_package").toString();
		String configFilePath = Paths.get(experimentDirPath, "property").toString();
		String intermediateDirPath = Paths.get(experimentDirPath, "Corpus").toString();
...
//		String outputFilePath=Paths.get(rootDirPath, "VSM", "output").toString();
		String outputFilePath = Paths.get(experimentDirPath, "output").toString();
		if(new File(outputFilePath).isFile()){
			new File(outputFilePath).delete();
		}
		
		String evaluationDirPath = Paths.get(experimentDirPath, "rankingResult").toString();
...
//			new File(intermediateDirPath).mkdir();
			for(String oneLogName:logNames){
				...
				Config.getInstance().setEvaluations(Paths.get(experimentDirPath, "eval").toString()/*Paths.get(rootDirPath, "VSM", "eval").toString()*/, true, true, 5, true);
				Config.getInstance().exportConfig(configFilePath);
				
				long runningTime=0;
				String resultFilePath=Paths.get(oneLogEvalDir,"ElementScoring").toString();
				runningTime=run(resultFilePath, "ElementScoring", propAlpha, propRho,lambda);
				...
				runningTime=run(resultFilePath, "Propagation", propAlpha, propRho,lambda);
				...
				runningTime=run(resultFilePath, "Aggregation", propAlpha, propRho,lambda);
				...
				runningTime=run(resultFilePath, "LM", propAlpha, propRho,lambda);
				FileUtils.write_append2file(pair.getKey()+" "+oneLogName+" "+"LM"+" "+runningTime+"\n", timestatsFilePath);

				resultFilePath=Paths.get(oneLogEvalDir,"VSM").toString();
				runningTime=run(resultFilePath, "VSM", propAlpha, propRho,lambda);
